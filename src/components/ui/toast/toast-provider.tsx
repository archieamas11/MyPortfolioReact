...
const ToastCard: React.FC<{
  ...
  useEffect(() => {
    ...
    const removeDelayMs = prefersReducedMotion
      ? 0
      ...
      <div className="relative p-3 pr-8">
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0 flex-1 space-y-1">
            ...
            {toast.description && (
              <p className="text-current/80 text-sm leading-snug">
                {toast.description}
              </p>
            )}
          </div>
          ...
          <button
            ...
            className="absolute top-1 right-1 flex h-4 w-4 cursor-pointer items-center justify-center rounded-full border border-white/25 bg-white/10 font-medium text-sm text-white transition duration-150 hover:bg-white/20 focus-visible:outline-2 focus-visible:outline-white/40 focus-visible:outline-offset-2"
      ...
      setToasts((prev) => [
        ...prev,
        {
          ...
          enableLiquidAnimation:
            toast.enableLiquidAnimation ?? enableLiquidAnimation,
  ...
  return (
    <ToastContext.Provider value={contextValue}>
      ...
      {portalNode &&
        createPortal(
          (
            Object.entries(toastsByPosition) as [ToastPosition, ToastRecord[]][]
