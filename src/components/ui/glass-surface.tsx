...
export type GlassSurfaceProps =
  | GlassSurfaceOverlayProps
  | GlassSurfaceInteractiveProps;
...
function buildEdgeGradients(intensity: number) {
  const intensitySafe = Number.isFinite(intensity)
...
const GlassSurfaceOverlay = React.forwardRef<
  HTMLDivElement,
  GlassSurfaceOverlayProps
...
const GlassSurfaceInteractive = React.forwardRef<
  HTMLElement,
  GlassSurfaceInteractiveProps
>(function GlassSurfaceInteractive(
  ...
  useEffect(() => {
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      elasticityFrameRef.current =
        window.requestAnimationFrame(applyElasticity);
    };
  ...
  if (asChild) {
    ...
    if (
      !(onlyChild && React.isValidElement<ChildInteractiveProps>(onlyChild))
    ) {
      return (
        <div
          className={cn(
    ...
    const typedChild = onlyChild as React.ReactElement<
      ChildInteractiveProps & React.RefAttributes<HTMLElement>
    >;
    const existingRef = (
      typedChild as unknown as { ref?: React.Ref<HTMLElement> }
    ).ref;
