...
export type UseElasticityOptions = {
  enabled?: boolean
  elasticity?: number
...
export function useElasticity<T extends HTMLElement>(
  ...
  const { enabled, elasticity, activationZonePx, transitionMs } = { ...DEFAULTS, ...options }
  ...
  const [scale, setScale] = React.useState<{ x: number; y: number } | null>(null)
