...
import { type ToastDefinition, useToast } from '@/components/ui/toast/toast-provider'
...
export function useChatbotApi({
  ...
  const maybeTriggerStreamingHaptic = useCallback(
    (chunkText: string) => {
      ...
      Promise.resolve(runHaptic(HAPTIC_DURATION_MS, { intensity: 0.15 })).catch(() => undefined)
    },
    [trigger],
  )
    ...
    async (input: string, isRateLimited: boolean) => {
      ...
      try {
        const response = await sendChatMessageStreaming(
          trimmed,
          handleStreamUpdate(botMessage.id, maybeTriggerStreamingHaptic, updateMessage),
          [],
          controller.signal,
