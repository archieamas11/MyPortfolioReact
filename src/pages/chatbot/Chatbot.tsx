...
export function Chatbot({ isMini = false }: ChatbotProps) {
  ...
  const { messages, addMessages, updateMessage, clearMessages } =
    useChatMessages();
  ...
  const handleSendMessage = useCallback(() => {
    ...
    const matchingTag = SUGGESTION_TAGS.find(
      (tag) => tag.question === trimmedInput
    );
    if (matchingTag) {
  ...
  const handleClearChat = useCallback(() => {
    ...
    const isAlreadyCleared =
      messages.length === 1 && messages[0].text === INITIAL_MESSAGE_TEXT;
    if (isAlreadyCleared) {
      return;
    }
