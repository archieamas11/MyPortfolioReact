...
function resolveNavigationIcon(
  item: NavItem,
  theme: string
): ResolvedNavigationIcon | null {
  ...
  if (isThemeToggle) {
    return {
      Icon: (theme === "dark"
        ? MoonIcon
        : SunIcon) as React.ForwardRefExoticComponent<
        NavIconProps & React.RefAttributes<IconHandle>
      >,
      isThemeToggle,
    };
...
function canRenderNavigationItem(
  item: NavItem,
  isThemeToggle: boolean
): boolean {
  if (!item.ariaLabel) {
        ...
        <Tooltip open={showTooltip ? undefined : false}>
          <TooltipTrigger asChild>
            <a
              ...
              className={cn(
                "group flex flex-col items-center justify-center rounded-lg text-center text-muted-foreground text-xs no-underline",
                ...
                {
                  "glass-effect bg-primary/5 text-accent backdrop-blur-3xl":
                    isActive,
            ...
            >
              <div
                className={cn(
                  "flex transform-gpu items-center justify-center outline-none transition-all duration-300 ease-in-out",
              ...
              <span
                className={cn(
                  "transform-gpu font-semibold text-[12px] transition-all delay-100 duration-300 ease-in-out",
                  "max-md:hidden max-[480px]:hidden",
                  {
                    "h-0 translate-y-2 scale-0 opacity-0": !isActive || isMini,
                    "mt-1 h-3 translate-y-0 scale-110 opacity-100":
                      isActive && !isMini,
                  }
                )}
              >
